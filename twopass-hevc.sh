...
printf "%s\n" "Enter audiobitrate (for example 5.1 (6 channels) Vorbis audio: 128Kbps x (6 / 2) = 384Kbps)";
read audiobitrate

printf "%s\n" "Enter preset, the preset determines how fast the encoding process will be [medium | slow | slowest | slower | veryslow | fast | superfast | faster | ultrafast]"
read preset
...
for name in *.$fileformat; do
    ...
    -x265-params pass=2:qcomp=0.8:aq-mode=1:aq_strength=1.0:qg-size=16:psy-rd=0.7:psy-rdoq=5.0:rdoq-level=1:merange=44 -max_muxing_queue_size 1024 -c:a aac -b:a "$audiobitrate"k "x265-converted/film_${name%.*}.mkv"
done
